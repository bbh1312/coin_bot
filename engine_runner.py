...
try:
    ...
    TierCoordinator = None
    TierCoordConfig = None
...
AtlasRsFailShortEngine = None
AtlasRsFailShortConfig = None
if "SwaggyAtlasLabEngine" not in globals():
    SwaggyAtlasLabEngine = None
if "SwaggyAtlasLabConfig" not in globals():
...
if "lab_evaluate_global_gate" not in globals():
    lab_evaluate_global_gate = None
if "lab_evaluate_local" not in globals():
    lab_evaluate_local = None
if "lab_apply_policy" not in globals():
...
if "DTFXConfig" not in globals():
    DTFXConfig = None
if "write_dtfx_log" not in globals():
    write_dtfx_log = None
if "st_flip_compute_signal" not in globals():
    st_flip_compute_signal = None
if "StFlipConfig" not in globals():
    StFlipConfig = None
...
BOT_TOKEN_SUPER_TREND = os.getenv("TELEGRAM_BOT_TOKEN_SUPER_TREND", "")
CHAT_ID_SUPER_TREND = os.getenv("TELEGRAM_CHAT_ID_SUPER_TREND", "")
...
try:
    if TierCoordinator is not None and TierCoordConfig is not None:
        _tc_cfg = TierCoordConfig(
            shared_cooldown_sec=int(float(os.getenv("TIER_SHARED_COOLDOWN_SEC", "2700"))),
            block_b_on_a_signal_sec=int(float(os.getenv("TIER_BLOCK_B_ON_A_SIGNAL_SEC", "5400"))),
            block_b_on_a_open_sec=int(float(os.getenv("TIER_BLOCK_B_ON_A_OPEN_SEC", "14400"))),
            max_open_b=int(float(os.getenv("TIER_MAX_OPEN_B", "4"))),
        )
        TIER_COORDINATOR = TierCoordinator(_tc_cfg)
except Exception:
    TIER_COORDINATOR = None
...
def short_market(
    ...
    include_inactive_followers: bool = False,
) -> dict:
    ...
    if not admin_ok:
        ...
        return res
    followers = _follower_contexts_for_sync(include_inactive=True) if include_inactive_followers else FOLLOWER_CONTEXTS
    if not followers:
        _set_last_entry_broadcast(symbol, "SHORT", admin_status, admin_ok, [])
        return res
    use_pct = _calc_effective_entry_pct(usdt_amount)
    follower_calls = []
    active_names = _active_account_names() if not include_inactive_followers else None
    for acct in followers:
        if (not include_inactive_followers) and (not _is_active_follower(acct, active_names=active_names)):
    ...
    results = _broadcast_followers(
        "short_market",
        follower_calls,
        {"symbol": symbol},
        enforce_active=not include_inactive_followers,
    )
...
def short_limit(*args, **kwargs) -> dict:
    ...
    for acct in followers:
        if not _is_active_follower(acct, active_names=active_names):
            follower_calls.append({"acct": acct, "skip": "inactive"})
            continue
        follower_usdt = usdt_amount
        if use_pct is not None:
...
def close_long_market(symbol: str) -> dict:
    try:
        ...
        caller = cf.f_back if cf is not None else None
        caller_fn = caller.f_code.co_name if caller is not None and caller.f_code else "unknown"
        caller_ln = caller.f_lineno if caller is not None else -1
        caller_locals = caller.f_locals if caller is not None else {}
        trace_parts = [
            f"caller={caller_fn}:{caller_ln}",
            f"engine_label={caller_locals.get('engine_label')}",
            f"is_srp_long_meta={caller_locals.get('is_srp_long_meta')}",
            f"exit_reason={caller_locals.get('exit_reason')}",
            f"profit={caller_locals.get('profit_unlev')}",
            f"mark={caller_locals.get('mark_px')}",
            f"entry={caller_locals.get('entry_px')}",
            f"sl_price={caller_locals.get('sl_price_meta')}",
            f"sl_pct={caller_locals.get('sl_pct')}",
            f"tp_pct={caller_locals.get('tp_pct')}",
        ]
        _append_entry_gate_log("close_long_trace", symbol, " ".join(trace_parts), side="LONG")
    except Exception:
        pass
    ...
    try:
        _append_entry_gate_log(
            "close_long_trace",
            symbol,
            f"result status={admin_status} order_id={_order_id_from_res(res)}",
            side="LONG",
        )
    except Exception:
        pass
...
def dca_short_if_needed(symbol: str, adds_done: int, margin_mode: str = "isolated") -> dict:
    res = _EXEC_DCA_SHORT_IF_NEEDED(symbol, adds_done, margin_mode)
    followers = _follower_contexts_for_sync(include_inactive=True)
    if not followers:
        return res
    follower_calls = []
    for acct in followers:
        try:
            follower_calls.append(
                {
                    "acct": acct,
                    "fn": lambda a=acct: a.executor.dca_short_if_needed(
                        symbol, adds_done=adds_done, margin_mode=margin_mode
                    ),
                }
            )
        except Exception:
            follower_calls.append({"acct": acct, "fn": None})
    _broadcast_followers(
        "dca_short_if_needed",
        follower_calls,
        {"symbol": symbol},
...
def dca_long_if_needed(symbol: str, adds_done: int, margin_mode: str = "isolated") -> dict:
    res = _EXEC_DCA_LONG_IF_NEEDED(symbol, adds_done, margin_mode)
    followers = _follower_contexts_for_sync(include_inactive=True)
    if not followers:
        return res
    follower_calls = []
    for acct in followers:
        try:
            follower_calls.append(
                {
                    "acct": acct,
                    "fn": lambda a=acct: a.executor.dca_long_if_needed(
                        symbol, adds_done=adds_done, margin_mode=margin_mode
                    ),
                }
            )
...
ADV_TREND_TP1_R_MULT = float(os.getenv("ADV_TREND_TP1_R_MULT", "1.4"))
...
SRP_LTF = "5m"
SRP_HTF = "1h"
ST_FLIP_LTF = os.getenv("ST_FLIP_LTF", "3m")
ST_FLIP_EMA_LEN = int(os.getenv("ST_FLIP_EMA_LEN", "60"))
ST_FLIP_EMA_LONG_LEN = int(os.getenv("ST_FLIP_EMA_LONG_LEN", "200"))
...
ST_FLIP_PSAR_MAX = float(os.getenv("ST_FLIP_PSAR_MAX", "0.2"))
ST_FLIP_VOL_SMA_LEN = int(os.getenv("ST_FLIP_VOL_SMA_LEN", "20"))
ST_FLIP_VOL_MULT = float(os.getenv("ST_FLIP_VOL_MULT", "1.5"))
ST_FLIP_RSI_LEN = int(os.getenv("ST_FLIP_RSI_LEN", "14"))
ST_FLIP_RSI_MIN = float(os.getenv("ST_FLIP_RSI_MIN", "30"))
ST_FLIP_RSI_MAX = float(os.getenv("ST_FLIP_RSI_MAX", "70"))
ST_FLIP_RSI_MOM_MIN = float(os.getenv("ST_FLIP_RSI_MOM_MIN", "0"))
ST_FLIP_BODY_PCT_MIN = float(os.getenv("ST_FLIP_BODY_PCT_MIN", "0"))
ST_FLIP_ADX_OFF = os.getenv("ST_FLIP_ADX_OFF", "0") == "1"
...
MANAGE_DCA_SR_GATE: bool = str(os.getenv("MANAGE_DCA_SR_GATE", "1")).strip().lower() not in ("0", "false", "off", "no")
MANAGE_DCA_SR_NEAR_ATR_MULT: float = float(os.getenv("MANAGE_DCA_SR_NEAR_ATR_MULT", "0.35"))
MANAGE_DCA_SR_ATR_LEN: int = int(os.getenv("MANAGE_DCA_SR_ATR_LEN", "14"))
MANAGE_DCA_SR_LOG_COOLDOWN_SEC: float = float(os.getenv("MANAGE_DCA_SR_LOG_COOLDOWN_SEC", "60"))
SUPPRESS_RECONCILE_ALERTS: bool = False
...
def _is_active_follower(acct: Optional[AccountContext], active_names: Optional[set] = None) -> bool:
    if not acct:
        return False
    names = active_names if isinstance(active_names, set) else _active_account_names()
    if not names:
        # fail-safe: if active account lookup fails, do not propagate follower orders.
        return False
    return str(acct.name) in names
...
COMMON_UNIVERSE_EXCLUDE_MCAP_TOP_N = int(os.getenv("COMMON_UNIVERSE_EXCLUDE_MCAP_TOP_N", "20"))
COMMON_UNIVERSE_MCAP_INCLUDE_BASES = {
    s.strip().upper()
    for s in os.getenv("COMMON_UNIVERSE_MCAP_INCLUDE_BASES", "BTC,ETH").split(",")
    if s.strip()
}
COMMON_UNIVERSE_MCAP_CACHE_TTL_SEC = int(os.getenv("COMMON_UNIVERSE_MCAP_CACHE_TTL_SEC", "21600"))
COMMON_UNIVERSE_FORCE_EXCLUDE = tuple(
    s.strip()
    for s in os.getenv("COMMON_UNIVERSE_FORCE_EXCLUDE", "TSLA/USDT:USDT,PAXG/USDT:USDT").split(",")
    if s.strip()
)
COMMON_UNIVERSE_MCAP_FALLBACK = tuple(
    s.strip().upper()
    for s in os.getenv(
        "COMMON_UNIVERSE_MCAP_FALLBACK",
        "BTC,ETH,SOL,BNB,XRP,ADA,DOGE,TRX,LINK,AVAX,DOT,SUI,LTC,XLM,HBAR,SHIB,TON,BCH,UNI,PEPE",
    ).split(",")
    if s.strip()
)
_COMMON_MCAP_CACHE = {"ts": 0.0, "bases": []}
...
def _fetch_top_market_cap_bases(limit: int) -> list[str]:
    if not isinstance(limit, int) or limit <= 0:
        return []
    now = time.time()
    cached = _COMMON_MCAP_CACHE.get("bases")
    cached_ts = float(_COMMON_MCAP_CACHE.get("ts", 0.0) or 0.0)
    if isinstance(cached, list) and cached and (now - cached_ts) < max(300, COMMON_UNIVERSE_MCAP_CACHE_TTL_SEC):
        return list(cached)
    bases: list[str] = []
    try:
        per_page = max(30, min(250, int(limit * 2)))
        url = (
            "https://api.coingecko.com/api/v3/coins/markets"
            f"?vs_currency=usd&order=market_cap_desc&per_page={per_page}&page=1&sparkline=false"
        )
        ...
        bases = []
    if not bases:
        bases = list(COMMON_UNIVERSE_MCAP_FALLBACK[:limit])
    out = []
    for b in bases:
        if b not in out:
            out.append(b)
        if len(out) >= limit:
            break
    _COMMON_MCAP_CACHE["ts"] = now
    _COMMON_MCAP_CACHE["bases"] = list(out)
    return list(out)
...
def _build_common_universe(tickers: dict, symbols: list) -> list:
    ...
    if COMMON_UNIVERSE_TOP_N:
        shared_top_n = COMMON_UNIVERSE_TOP_N
    excluded_bases = set()
    if COMMON_UNIVERSE_EXCLUDE_MCAP_TOP_N > 0:
        need_n = int(COMMON_UNIVERSE_EXCLUDE_MCAP_TOP_N)
        raw_limit = need_n + max(10, len(COMMON_UNIVERSE_MCAP_INCLUDE_BASES) * 3)
        top_bases = _fetch_top_market_cap_bases(raw_limit)
        excluded_list = []
        for b in top_bases:
            if not b or b in COMMON_UNIVERSE_MCAP_INCLUDE_BASES:
                continue
            if b in excluded_list:
                continue
            excluded_list.append(b)
            if len(excluded_list) >= need_n:
                break
        excluded_bases = set(excluded_list)
    ...
    shared_universe = _exclude_common_symbols(shared_universe)
...
def _warmup_common_cache(state: dict, exchange, universe: list) -> bool:
    ...
    while idx < total and fetch_count < COMMON_WARMUP_MAX_FETCH:
        ...
        # allow one-bar lag for warmup freshness check
        file_stale = bool(tf_ms and last_ts and last_ts < (expected_last_open - tf_ms))
        if isinstance(raw, list) and len(raw) >= limit and file_has and not file_gap and not file_stale:
    ...
    if done_all and not missing_pass:
        ...
        for sym, tf, limit in plan:
            ...
            expected_last_open = (now_ms // tf_ms) * tf_ms if tf_ms else 0
            file_stale = bool(tf_ms and last_ts and last_ts < (expected_last_open - tf_ms))
            if not isinstance(raw, list) or len(raw) < limit or not file_has or file_gap or file_stale:
...
def _reset_common_warmup_state(state: dict) -> None:
    ...
    if isinstance(meta, dict):
        ...
        meta.pop("common_warmup_notified_day", None)
        meta.pop("common_warmup_last_notify_ts", None)

def _maybe_daily_refresh_common_universe(state: dict, tickers: dict, symbols: list) -> bool:
    if not COMMON_UNIVERSE_REFRESH_ENABLED:
        return False
    try:
        now_kst = _kst_now()
        ...
        return False
    try:
        refreshed = _build_common_universe(tickers, symbols)
        state["_common_universe"] = list(refreshed)
        state["_common_universe_ready"] = True
        state["_common_universe_refresh_date"] = today
        if COMMON_UNIVERSE_LOG_PATH and refreshed:
            try:
                _append_log_lines(COMMON_UNIVERSE_LOG_PATH, [f"COMMON_UNIVERSE daily_refresh size={len(refreshed)}"])
                _append_log_lines(COMMON_UNIVERSE_LOG_PATH, refreshed)
            except Exception:
                pass
        try:
        ...
        print(f"[common-universe] daily refresh failed: {e}")
        return False
...
def _hydrate_open_trade_meta(state: Dict[str, dict], now_ts: Optional[float] = None) -> bool:
    ...
    for tr in open_trades:
        ...
        if engine_label == "SR_PRO_SHORT_V1" and entry_px:
            try:
...
def _append_sr_pro_short_v1_log(line: str) -> None:
    date_tag = time.strftime("%Y-%m-%d")
    ts = time.strftime("%Y-%m-%d %H:%M:%S")
...
def _append_sr_pro_long_v1_log(line: str) -> None:
    try:
        date_tag = time.strftime("%Y-%m-%d")
        ts = time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    for symbol in swaggy_universe:
        ...
        if not in_pos_same:
            ...
            if _exit_cooldown_blocked(state, symbol, "swaggy_no_atlas", side, ttl_sec=COOLDOWN_SEC):
                ...
                continue
        if in_pos_same:
            open_tr_same = _get_open_trade(state, side, symbol)
            open_engine_same = _engine_label_from_reason(
                (open_tr_same.get("meta") or {}).get("reason")
            ) if isinstance(open_tr_same, dict) else ""
            # SWAGGY_NO_ATLAS should not manage/close positions opened by other engines.
            if open_engine_same and open_engine_same != "SWAGGY_NO_ATLAS":
                if isinstance(st.get("_no_atlas_weak"), dict):
                    st.pop("_no_atlas_weak", None)
                    state[symbol] = st
                _append_swaggy_no_atlas_log(
                    f"SWAGGY_NO_ATLAS_SKIP sym={symbol} reason=FOREIGN_ENGINE_IN_POS engine={open_engine_same} side={side}"
                )
                time.sleep(PER_SYMBOL_SLEEP)
                continue
...
def _enqueue_entry_request(
    ...
    if TIER_COORDINATOR is not None and tier == "A":
        try:
            if not TIER_COORDINATOR.can_enter_a(symbol, now_ts):
                _append_entry_gate_log(engine.lower(), symbol, "tier_block=A", side=side)
                return None
        except Exception:
            pass
    if TIER_COORDINATOR is not None and tier == "B":
        try:
            if not TIER_COORDINATOR.can_enter_b(symbol, now_ts):
                _append_entry_gate_log(engine.lower(), symbol, "tier_block=B", side=side)
                return None
        except Exception:
            pass
    if _entry_blocked_now(ENTRY_BLOCK_HOURS):
...
def _log_trade_entry(
    ...
    if TIER_COORDINATOR is not None:
        try:
            ...
            if tier in ("A", "B"):
                TIER_COORDINATOR.on_open(tier, symbol, float(entry_ts))
        except Exception:
            pass
    if dbrec:
        try:
...
def _is_runtime_managed_engine(engine: Optional[str]) -> bool:
    key = (engine or "").upper()
    return key in (
        "SR_PRO_SHORT_V1",
        "SR_PRO_SHORT_V2",
...
def _tier_from_engine_label(engine_label: Optional[str]) -> Optional[str]:
    key = (engine_label or "").strip().upper()
    if key == "VERTICAL_EXHAUSTION_TRAP_SHORT":
        return "A"
    if key == "SCOUT_ONLY_EXHAUSTION_SHORT":
        ...
        if tr.get("side") == side and tr.get("symbol") == symbol and tr.get("status") == "open":
            ...
            if TIER_COORDINATOR is not None:
                try:
                    tier = _tier_from_engine_label(tr.get("engine_label"))
                    if tier in ("A", "B"):
                        TIER_COORDINATOR.on_close(tier, symbol, float(exit_ts))
                except Exception:
                    pass
...
def _run_sr_pro_short_v1_cycle(
    sr_universe,
    state,
    send_alert,
    cycle_id: Optional[int] = None,
    ...
    def _expected_last_closed_ts(tf: str, now_ms: Optional[int] = None) -> int:
        tf_ms = _tf_ms(tf)
        cur_ms = int(time.time() * 1000) if now_ms is None else int(now_ms)
        return int((cur_ms // tf_ms) * tf_ms - tf_ms)

    def _need_boundary_refresh(df: pd.DataFrame, tf: str, now_ms: Optional[int] = None) -> bool:
        if df is None or df.empty:
            return False
        tf_ms = _tf_ms(tf)
        cur_ms = int(time.time() * 1000) if now_ms is None else int(now_ms)
        # only in early boundary window; avoid extra fetch every cycle
        phase_ms = cur_ms % tf_ms
        if phase_ms > 45_000:
            return False
    ...
    for symbol in symbols:
        ...
        dvf_prev = (
            float(dvf.iloc[idx_1h - 1]) / float(vol_ema.iloc[idx_1h - 1])
            if idx_1h - 1 >= 0 and float(vol_ema.iloc[idx_1h - 1]) > 0
            else dvf_norm
        )
        ...
        try:
            ...
            if idx_15m >= 1 and len(ema60) > idx_15m and len(ema120) > idx_15m:
        ...
        if getattr(cfg, "aggr_break_enabled", False) and strong_break:
            aggr_body_ok = body_ratio >= float(getattr(cfg, "aggr_body_ratio_min", 0.6))
            aggr_dist_ok = (atr_now > 0) and ((c3 - high_max) >= (atr_now * float(getattr(cfg, "aggr_close_atr_min", 0.1))))
            ema20_3m = ema(df_3m_sig["close"], 20)
            ema20_now_3m = float(ema20_3m.iloc[-1]) if not np.isnan(ema20_3m.iloc[-1]) else c3
            if aggr_body_ok and aggr_dist_ok and (c3 > o3) and (c3 > ema20_now_3m):
                gate_stats["aggr_break"] += 1
                entry_px = float(c3)
                if atr_now > 0 and entry_px > (c3 + (atr_now * float(getattr(cfg, "aggr_entry_cap_atr", 0.3)))):
                    gate_stats["aggr_cap_reject"] += 1
                else:
                    sl_price = entry_px - (atr_now * float(getattr(cfg, "aggr_sl_atr_mult", 1.0)))
            ...
            if retest_touch:
                ...
                try:
                    _append_sr_pro_short_v1_log(
                        ...
                        f"h15_0={h15_0:.6f} h15_1={h15_1:.6f} h15_2={h15_2:.6f} "
                        f"c3={c3:.6f} o3={o3:.6f} h3={h3:.6f} l3={l3:.6f} "
                        f"low_min={low_min:.6f} strong={int(strong_break)} weak={int(weak_break)} "
                        f"retest_level={retest_level:.6f} retest_until={retest_until}"
                    )
                except Exception:
                    pass
                req_id = _enqueue_entry_request(
...
def _run_sr_pro_long_v1_cycle(
    ...
    log_fn(
        ...
        f"skip_stale_ts={gate_stats['skip_stale_ts']} "
        f"cooldown={gate_stats['cooldown']} "
    ...
    for tr in open_longs:
        ...
        if isinstance(meta, dict):
            try:
                ...
                sl_price_meta = None
            try:
                tp_price_meta = float(meta.get("tp_price"))
            except Exception:
                tp_price_meta = None
        if isinstance(meta, dict) and meta.get("sl_order_id") and isinstance(exit_price, (int, float)):
            if isinstance(sl_price_meta, (int, float)) and exit_price <= float(sl_price_meta):
        ...
        if exit_reason == "manual_close":
            ...
            if isinstance(entry_price, (int, float)) and isinstance(exit_price, (int, float)) and entry_price > 0:
                ...
                elif isinstance(tp_pct, (int, float)) and profit_unlev >= float(tp_pct):
                    exit_reason = "auto_exit_tp"
        ...
        if (
            ...
            send_telegram(
                ...
                f"사유: {_telegram_reason_text(engine_label, exit_tag)}\n"
                f"{entry_line}"
                f"청산가={price_str}\n"
                f"손익={pnl_str}"
            )
...
def _process_manage_queue(state: dict, send_telegram) -> None:
    ...
    for req in reqs:
        ...
        if engine and engine not in allowed_engines:
            ...
            if os.getenv("MANAGE_QUEUE_LOG_SKIPS", "0") == "1":
        ...
        symbol = str(req.get("symbol") or "")
        side = str(req.get("side") or "").upper()
        if engine in ("SWAGGY_NO_ATLAS", "SWAGGY_ATLAS_LAB"):
            ...
            if engine == "SWAGGY_ATLAS_LAB" and _is_in_off_window(SWAGGY_ATLAS_LAB_OFF_WINDOWS, now):
                ...
                status[req_id] = {"status": "failed", "ts": time.time(), "reason": "off_window"}
                continue
        ok, reason = _execute_manage_entry_request(state, req, send_telegram)
        status[req_id] = {"status": "done" if ok else "failed", "ts": time.time(), "reason": reason}
...
def _detect_manual_positions(state: dict, send_telegram) -> None:
    ...
    for side_key, side_label in (("short", "SHORT"), ("long", "LONG")):
        ...
        for sym in syms:
            ...
            if seen_key in seen:
                # Track size increase for already-seen manual positions and alert as DCA.
                st_seen = state.get(sym) if isinstance(state.get(sym), dict) else {}
                if not isinstance(st_seen, dict):
                    st_seen = {}
                prev_qty_seen = st_seen.get(f"manual_qty_{side_key}")
                open_tr_seen = _get_open_trade(state, side_label, sym)
                meta_seen = (open_tr_seen or {}).get("meta") if isinstance(open_tr_seen, dict) else {}
                is_manual_trade = isinstance(meta_seen, dict) and str(meta_seen.get("reason") or "").lower() == "manual_entry"
                if is_manual_trade and isinstance(prev_qty_seen, (int, float)):
                    detail_seen = get_short_position_detail(sym) if side_label == "SHORT" else get_long_position_detail(sym)
                    if isinstance(detail_seen, dict):
                        qty_seen = detail_seen.get("qty") or detail_seen.get("amount")
                        entry_seen = detail_seen.get("entry")
                        mark_seen = detail_seen.get("mark")
                        try:
                            qty_seen = float(qty_seen)
                        except Exception:
                            qty_seen = None
                        ...
                        if isinstance(qty_seen, (int, float)) and qty_seen > float(prev_qty_seen) * 1.0001:
                            ...
                            send_telegram(
                                ...
                                f"mark={mark_seen} entry={entry_seen} qty={qty_seen}"
                            )
                            try:
                                save_state(state)
                            except Exception:
            ...
            if manual_followers:
                if _entry_blocked_now(ENTRY_BLOCK_HOURS):
                    ...
                    if follower_calls:
                        _broadcast_followers(
                            "long_market" if side_label == "LONG" else "short_market",
                            follower_calls,
                            {"symbol": sym},
                            enforce_active=False,
                        )
...
def _execute_manage_entry_request(state: dict, req: dict, send_telegram) -> tuple[bool, str]:
    ...
    try:
        if side == "SHORT":
            res = short_market(
                ...
                leverage=LEVERAGE,
                margin_mode=MARGIN_MODE,
                include_inactive_followers=manual_force_follow,
            )
        else:
            res = long_market(
                symbol,
    ...
    if side == "LONG" and engine_upper in ("SR_PRO_LONG_V1", "SR_PRO_LONG_V2"):
        ...
        try:
            ...
            if not isinstance(st, dict):
                st = {}
            st.pop("_srp_long_sl_order_id", None)
            state[symbol] = st
        except Exception:
            pass
        if not isinstance(sl_price_meta, (int, float)) or sl_price_meta <= 0:
            ...
            try:
                ...
                if isinstance(res_sl, dict) and res_sl.get("status") != "ok":
                    ...
                    sl_order_id = _order_id_from_res(res_sl) if isinstance(res_sl, dict) else None
                    if isinstance(sl_order_id, str) and sl_order_id:
                        try:
                            meta["sl_order_id"] = sl_order_id
                            open_tr = _get_open_trade(state, "LONG", symbol)
                            if isinstance(open_tr, dict):
                                open_meta = open_tr.get("meta") if isinstance(open_tr.get("meta"), dict) else {}
                                if not isinstance(open_meta, dict):
                                    open_meta = {}
                                open_meta["sl_order_id"] = sl_order_id
                                open_tr["meta"] = open_meta
                            st = state.get(symbol) if isinstance(state.get(symbol), dict) else {}
                            if not isinstance(st, dict):
                                st = {}
                            st["_srp_long_sl_order_id"] = sl_order_id
                            state[symbol] = st
                        except Exception:
                            pass
                    _append_entry_gate_log(
    ...
    if isinstance(recent_alert, dict):
        ...
        try:
            ...
            recent_ts = 0.0
        recent_engine = str(recent_alert.get("engine") or "").upper()
        cur_engine = str(req.get("engine") or "").upper()
        if recent_ts and (time.time() - recent_ts) <= 120.0 and (not cur_engine or cur_engine == recent_engine):
            _clear_manage_pending(state, symbol, side)
            return True, "duplicate_alert_skipped"
    ...
    for tr in open_trades:
        ...
        if not isinstance(amt, (int, float)) or amt <= 0:
            ...
            send_telegram(
                ...
                f"<b>{symbol}</b>\n"
                f"엔진: {_display_engine_label(engine_label)}\n"
                f"사유: {_telegram_reason_text(engine_label, _display_engine_label(engine_label))}"
            )
            continue
...
def _confirmed_df_by_now(df: pd.DataFrame, tf: str, now_ts: Optional[float] = None) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    try:
        tf_ms = _tf_ms(tf)
        now_ms = int((float(now_ts) if isinstance(now_ts, (int, float)) else time.time()) * 1000)
        last_ts = int(df.iloc[-1]["ts"]) if "ts" in df.columns else 0
        if last_ts and (now_ms - last_ts) < tf_ms:
            return df.iloc[:-1]
    except Exception:
...
def _zone_distance_to_price(price: float, zone: dict) -> float:
    try:
        top = float(zone.get("top"))
        bot = float(zone.get("bot"))
    except Exception:
        return float("inf")
    if bot > top:
        bot, top = top, bot
...
def _dca_sr_gate_should_log(st: dict, side: str, now_ts: float) -> bool:
    if not isinstance(st, dict):
        return False
    key = "_dca_sr_gate_log_ts_long" if str(side).upper() == "LONG" else "_dca_sr_gate_log_ts_short"
    last = float(st.get(key, 0.0) or 0.0)
    if (now_ts - last) >= max(1.0, float(MANAGE_DCA_SR_LOG_COOLDOWN_SEC)):
        st[key] = now_ts
        return True
    return False

def _dca_sr_zone_gate(symbol: str, side: str, now_ts: Optional[float] = None) -> tuple[bool, str]:
    side_u = str(side or "").upper()
    if not MANAGE_DCA_SR_GATE:
        return True, "disabled"
    if side_u not in ("LONG", "SHORT"):
        return False, "bad_side"
    if build_sr_zones is None:
        return True, "sr_builder_missing"
    cfg = SrProLongV1Config() if side_u == "LONG" else SrProShortV1Config()
    if cfg is None:
        return True, "sr_cfg_missing"
    try:
        window_bars_1h = max(0, int(cfg.total_window_days) * 24) if cfg.rolling_zones else 0
        min_htf = max(int(cfg.lookback) * 2 + 50, 220, window_bars_1h + 5 if window_bars_1h else 0)
        df_1h = cycle_cache.get_df(symbol, str(cfg.tf_htf), limit=min_htf + 2)
        df_3m = cycle_cache.get_df(symbol, str(cfg.tf_ltf), limit=max(80, int(MANAGE_DCA_SR_ATR_LEN) * 3))
        df_1h = _confirmed_df_by_now(df_1h, str(cfg.tf_htf), now_ts=now_ts)
        df_3m = _confirmed_df_by_now(df_3m, str(cfg.tf_ltf), now_ts=now_ts)
        if df_1h is None or df_3m is None or len(df_1h) < 30 or len(df_3m) < max(20, int(MANAGE_DCA_SR_ATR_LEN) + 2):
            return False, "no_data"
        zones = build_sr_zones(df_1h, cfg, window_bars=window_bars_1h)
        if not zones:
            return False, "no_zones"
        side_flag = -1 if side_u == "LONG" else 1
        side_zones = [z for z in zones if int(z.get("side", 0)) == side_flag]
        if not side_zones:
            return False, "no_side_zones"
        price = float(df_3m.iloc[-1]["close"])
        atr_val = float(atr(df_3m, int(MANAGE_DCA_SR_ATR_LEN)).iloc[-1])
        if not np.isfinite(atr_val) or atr_val <= 0:
            return False, "atr_nan"
        band = atr_val * float(MANAGE_DCA_SR_NEAR_ATR_MULT)
        if band <= 0:
            return True, "band_off"
        nearest = min(side_zones, key=lambda z: _zone_distance_to_price(price, z))
        dist = _zone_distance_to_price(price, nearest)
        if side_u == "LONG":
            # Support should be at/under current price (or very near above) for averaging down.
            cond = dist <= band and float(nearest.get("top", price)) <= (price + band)
        else:
            # Resistance should be at/above current price (or very near below) for averaging up.
            cond = dist <= band and float(nearest.get("bot", price)) >= (price - band)
        if cond:
            return True, f"ok dist={dist:.6g} band={band:.6g}"
        return False, f"far dist={dist:.6g} band={band:.6g}"
    except Exception as e:
...
def _run_manage_cycle(state: dict, exchange, cached_long_ex, send_telegram) -> None:
    ...
    for sym, st in list(state.items()):
        ...
        sr_gate_ok, sr_gate_reason = _dca_sr_zone_gate(sym, "SHORT", now_ts=now)
        if not sr_gate_ok:
            if _dca_sr_gate_should_log(st, "SHORT", now):
                _append_entry_gate_log("manage_dca", sym, f"sr_gate_block {sr_gate_reason}", side="SHORT")
                state[sym] = st
            continue
        ...
        if dca_res.get("status") not in ("skip", "warn"):
            ...
            if (not dca_key_alerted) or is_new_order:
                ...
                if order_id:
                    st["last_dca_order_id_short"] = order_id
                state[sym] = st
    ...
    for sym in long_syms:
            ...
            if (now - float(pending_ts)) >= 3.0:
                ...
                if isinstance(amt, (int, float)) and amt > 0:
                    try:
                        ...
                        if isinstance(res_sl, dict) and res_sl.get("status") == "ok":
                            sl_order_id = _order_id_from_res(res_sl)
                            try:
                                open_tr_cur = _get_open_trade(state, "LONG", sym)
                                if isinstance(open_tr_cur, dict):
                                    open_meta = open_tr_cur.get("meta") if isinstance(open_tr_cur.get("meta"), dict) else {}
                                    if isinstance(open_meta, dict):
                                        if isinstance(sl_order_id, str) and sl_order_id:
                                            open_meta["sl_order_id"] = sl_order_id
                                        open_tr_cur["meta"] = open_meta
                                        _set_open_trade(state, "LONG", sym, open_tr_cur)
                            except Exception:
                                pass
        ...
        if isinstance(meta, dict) and str(meta.get("reason") or "").lower() in ("sr_pro_long_v1", "sr_pro_long_v2"):
            ...
            if detail:
                mark_px = detail.get("mark")
            bars_since = None
            if entry_ts and isinstance(entry_ts, (int, float)):
                bars_since = int((now - float(entry_ts)) / 180.0)
            entry_dt = None
            if entry_ts and isinstance(entry_ts, (int, float)):
                try:
                    entry_dt = datetime.fromtimestamp(float(entry_ts)).strftime("%Y-%m-%d %H:%M:%S")
                except Exception:
                    entry_dt = None
            if (
                entry_ts
                and isinstance(mark_px, (int, float))
                and isinstance(fast_fail_level, (int, float))
                and (now - entry_ts) <= 180.0
                and (
                    float(mark_px)
                    < float(fast_fail_level)
                    - (
                        0.05 * float(fast_fail_atr)
                        if isinstance(fast_fail_atr, (int, float)) and float(fast_fail_atr) > 0
                        else 0.0
                    )
                )
            ):
                gap_atr = None
                if isinstance(fast_fail_atr, (int, float)) and float(fast_fail_atr) > 0:
                    gap_atr = (float(mark_px) - float(fast_fail_level)) / float(fast_fail_atr)
                _append_entry_gate_log(
                    "auto_exit_long",
                    sym,
                    f"fast_fail mark={mark_px} retest_level={fast_fail_level} "
                    f"gap_atr={gap_atr} entry_ts={entry_ts} entry_dt={entry_dt} "
                    f"bars_since_entry={bars_since}",
                    side="LONG",
                )
                gate_stats["fast_fail_count"] = gate_stats.get("fast_fail_count", 0) + 1
                try:
        ...
        if isinstance(open_tr, dict):
            ...
            if isinstance(tp_pct_meta, (int, float)):
                tp_pct = float(tp_pct_meta)
            try:
                reason_v = str(meta.get("reason") or "").strip().lower()
            except Exception:
                reason_v = ""
            try:
                engine_v = str(meta.get("engine") or "").strip().upper()
            except Exception:
                engine_v = ""
            is_srp_long_meta = (
            ...
            try:
                ...
                sl_price_meta = None
        if not is_srp_long_meta:
            st_fallback = state.get(sym) if isinstance(state.get(sym), dict) else {}
            if isinstance(st_fallback, dict):
                if isinstance(st_fallback.get("_srp_long_sl_order_id"), str) and st_fallback.get("_srp_long_sl_order_id"):
                    is_srp_long_meta = True
                elif isinstance(st_fallback.get("_srp_long_sl_price"), (int, float)):
                    is_srp_long_meta = True
        if is_srp_long_meta:
            tp_price_meta = None
            try:
        ...
        if AUTO_EXIT_ENABLED and profit_unlev >= tp_pct:
            ...
            if is_srp_long_meta:
                meta_rt = open_tr.get("meta") if isinstance(open_tr, dict) else None
                st_rt = state.get(sym) if isinstance(state.get(sym), dict) else {}
                sl_oid_meta = meta_rt.get("sl_order_id") if isinstance(meta_rt, dict) else None
                sl_oid_state = st_rt.get("_srp_long_sl_order_id") if isinstance(st_rt, dict) else None
                if sl_oid_meta or sl_oid_state:
                    _append_entry_gate_log(
                        "auto_exit_long",
                        sym,
                        (
                            "srp_guard_skip_local_sl_exchange_order "
                            f"mark={mark_px} sl={sl_price_meta} "
                            f"sl_oid_meta={sl_oid_meta} sl_oid_state={sl_oid_state}"
                        ),
                        side="LONG",
                    )
                    continue
                # SR_PRO_LONG_V1: 로컬 시장가 SL 청산은 사용하지 않고, 거래소 SL 재설치만 시도
                try:
                    amt = get_long_position_amount(sym)
                except Exception:
                    amt = None
                if isinstance(amt, (int, float)) and amt > 0:
                    try:
                        res_sl = place_long_sl_px(sym, float(sl_price_meta), qty=amt)
                        sl_order_id = _order_id_from_res(res_sl) if isinstance(res_sl, dict) else None
                        if isinstance(sl_order_id, str) and sl_order_id:
                            try:
                                meta_rt2 = open_tr.get("meta") if isinstance(open_tr, dict) and isinstance(open_tr.get("meta"), dict) else {}
                                if isinstance(meta_rt2, dict):
                                    meta_rt2["sl_order_id"] = sl_order_id
                                    open_tr["meta"] = meta_rt2
                                st2 = state.get(sym) if isinstance(state.get(sym), dict) else {}
                                if not isinstance(st2, dict):
                                    st2 = {}
                                st2["_srp_long_sl_order_id"] = sl_order_id
                                state[sym] = st2
                            except Exception:
                                pass
                            _append_entry_gate_log(
                                ...
                                sym,
                                (
                                    "srp_guard_rearm_exchange_sl "
                                    f"mark={mark_px} sl={sl_price_meta} sl_order_id={sl_order_id}"
                                ),
                                side="LONG",
                            )
                            continue
                ...
                _append_entry_gate_log(
                    ...
                    sym,
                    (
                        "srp_guard_hard_block_local_sl_no_exchange_order "
                        f"mark={mark_px} sl={sl_price_meta}"
                    ),
                    side="LONG",
                )
            ...
            if is_srp_long_meta:
                _append_entry_gate_log(
                    ...
                    sym,
                    (
                        "srp_guard_hard_block_local_sl_pct "
                        f"mark={mark_px} sl_pct={sl_pct} profit={profit_unlev:.4f}%"
                    ),
                    side="LONG",
                )
                continue
        ...
        if not closed:
            ...
            if (now - last_eval) >= MANAGE_EVAL_COOLDOWN_SEC:
                ...
                sr_gate_ok, sr_gate_reason = _dca_sr_zone_gate(sym, "LONG", now_ts=now)
                if not sr_gate_ok:
                    if _dca_sr_gate_should_log(st, "LONG", now):
                        _append_entry_gate_log("manage_dca", sym, f"sr_gate_block {sr_gate_reason}", side="LONG")
                        state[sym] = st
                    continue
                ...
                if dca_res.get("status") not in ("skip", "warn"):
                    ...
                    if (not dca_key_alerted) or is_new_order:
                        ...
                        if order_id:
                            st["last_dca_order_id_long"] = order_id
                        state[sym] = st
                        qty = dca_res.get("amount") or (dca_res.get("order") or {}).get("amount")
                        _record_position_event(
                            sym,
...
def _reload_runtime_settings_from_disk(state: dict, state_path: Optional[str] = None, skip_keys: Optional[set] = None) -> None:
    ...
    global SWAGGY_NO_ATLAS_ENABLED, SWAGGY_NO_ATLAS_OVEREXT_ENTRY_MIN
...
def _send_super_trend_alert(text: str, allow_early: bool = False) -> bool:
    if (time.time() - START_TIME) < TELEGRAM_STARTUP_GRACE_SEC and not allow_early:
        return False
    token = BOT_TOKEN_SUPER_TREND
    chat_id = CHAT_ID_SUPER_TREND
    if not token or not chat_id:
        print("[telegram-super-trend] missing TELEGRAM_BOT_TOKEN_SUPER_TREND or TELEGRAM_CHAT_ID_SUPER_TREND")
        return False
    ...
    for attempt in range(2):
        try:
            r = requests.post(url, json=payload, timeout=20)
            if not r.ok:
                print("[telegram-super-trend] failed:", r.status_code, r.text)
            return r.ok
        except Exception as e:
            if attempt == 1:
                print("[telegram-super-trend] error:", e)
                return False
            time.sleep(0.8)
    return False
            ...
            try:
                                ...
                                if isinstance(open_tr, dict):
                                    ...
                                    try:
                                        ...
                                        if isinstance(sl_price_meta, (int, float)) and isinstance(entry_px, (int, float)) and entry_px > 0:
                                            sl_calc = (float(sl_price_meta) - float(entry_px)) / float(entry_px) * 100.0
                                            if sl_calc > 0:
                                                sl_pct = sl_calc
                                    except Exception:
                                        pass
                ...
                if (cmd in ("/realtime_only", "realtime_only")) and not responded:
                    ...
                    if arg in ("off", "0", "false", "disable", "disabled"):
                        resp = "ℹ️ heavy-scan은 현재 비활성 고정입니다. realtime_only는 ON으로 유지됩니다."
                    elif arg in ("on", "1", "true", "enable", "enabled"):
                        resp = "✅ realtime_only ON (heavy-scan 비활성 고정)"
                    else:
                        resp = "ℹ️ realtime_only 상태: ON (heavy-scan 비활성 고정)"
                    if resp:
                        ok = _reply(resp)
                ...
                if (cmd in ("/max_pos", "max_pos")) and not responded:
                    ...
                    if resp:
                        ...
                        responded = True
                if not responded:
                    cmd_norm = (cmd or "").lstrip("/")
                    if cmd_norm in {
                        "swaggy_atlas_lab",
                        "swaggy_atlas_lab_off",
                        "swaggy_no_atlas",
                        "swaggy_no_atlas_off",
                        "swaggy_no_atlas_overext",
                        "swaggy_no_atlas_overext_on",
                        "swaggy_d1_overext",
                        "adv_trend",
                        "anti_alpha_v1",
                        "rsi",
                ...
                if (cmd in ("/anti_alpha_v1", "anti_alpha_v1")) and not responded:
                    ...
                    if resp:
                        ...
                        responded = True
                if (cmd in ("/sr_pro_short_v1", "sr_pro_short_v1", "sr_pro_short")) and not responded:
                    parts = lower.split()
                    arg = parts[1] if len(parts) >= 2 else "status"
                    resp = None
                    if arg in ("on", "1", "true", "enable", "enabled"):
                        SR_PRO_SHORT_V1_ENABLED = True
                        state["_sr_pro_short_v1_enabled"] = True
                        state_dirty = True
                        resp = "✅ sr_pro_short_v1 ON"
                    elif arg in ("off", "0", "false", "disable", "disabled"):
                        SR_PRO_SHORT_V1_ENABLED = False
                        state["_sr_pro_short_v1_enabled"] = False
                        state_dirty = True
                        resp = "⛔ sr_pro_short_v1 OFF"
                    else:
                        resp = (
                            f"ℹ️ sr_pro_short_v1 상태: {'ON' if SR_PRO_SHORT_V1_ENABLED else 'OFF'}\n"
                            "사용법: /sr_pro_short_v1 on|off|status"
                        )
                    if resp:
                        ok = _reply(resp)
                ...
                if (cmd in ("/sr_pro_long_v1", "sr_pro_long_v1", "sr_pro_long")) and not responded:
                    ...
                    resp = None
                    if arg in ("on", "1", "true", "enable", "enabled"):
                        SR_PRO_LONG_V1_ENABLED = True
                        state["_sr_pro_long_v1_enabled"] = True
                        state_dirty = True
                        resp = "✅ sr_pro_long_v1 ON"
                    elif arg in ("off", "0", "false", "disable", "disabled"):
                        SR_PRO_LONG_V1_ENABLED = False
                        state["_sr_pro_long_v1_enabled"] = False
                        state_dirty = True
                        resp = "⛔ sr_pro_long_v1 OFF"
                    else:
                        resp = (
                            f"ℹ️ sr_pro_long_v1 상태: {'ON' if SR_PRO_LONG_V1_ENABLED else 'OFF'}\n"
                            "사용법: /sr_pro_long_v1 on|off|status"
                        )
                    if resp:
                        ok = _reply(resp)
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            ...
            if isinstance(data, dict):
                _sanitize_state_meta_keys(data)
                return data
            return {}
    except Exception:
        return {}

def _read_common_universe_latest() -> List[str]:
    try:
        path = os.path.join("logs", "common_universe", "latest.txt")
        if not os.path.exists(path):
            return []
        with open(path, "r", encoding="utf-8") as f:
            out = [line.strip() for line in f.read().splitlines() if line.strip()]
        return out
    except Exception:
        return []

def _sanitize_state_meta_keys(state: Dict[str, Any]) -> None:
    if not isinstance(state, dict):
        return
    # Must remain list[str] for common-universe restore/reuse logic.
    cu = state.get("_common_universe")
    if not isinstance(cu, list):
        latest = _read_common_universe_latest()
        state["_common_universe"] = latest if latest else []
    else:
        state["_common_universe"] = [s for s in cu if isinstance(s, str) and s]
    # These control flow flags must be bool; polluted dict/object values break logic.
    for k in ("_common_universe_ready", "_common_warmup_done"):
        v = state.get(k)
        if isinstance(v, bool):
            continue
        if isinstance(v, (int, float, str)):
            state[k] = _coerce_bool(v)
        else:
            state[k] = False
...
def save_state(state: Dict[str, dict]) -> None:
    ...
    if not ENGINE_WRITE_STATE:
        return
    _sanitize_state_meta_keys(state)
    disk = None
    try:
        with open(path, "r", encoding="utf-8") as f:
            disk = json.load(f)
...
def run():
    ...
    try:
        ...
        pass
    try:
        def _cycle_cache_fetcher(sym: str, tf: str, limit: int):
            if isinstance(limit, int) and limit > 1500:
    ...
    global swaggy_engine, swaggy_atlas_lab_engine, swaggy_no_atlas_engine
    ...
    if isinstance(state.get("_sr_pro_short_v1_enabled"), bool):
        SR_PRO_SHORT_V1_ENABLED = bool(state.get("_sr_pro_short_v1_enabled"))
    else:
        ...
        for account_ctx in active_accounts:
            ...
            with (account_ctx.executor.activate() if account_ctx else nullcontext()):
                with _use_account_context(account_ctx):
                    ...
                    # Heavy-scan mode is fully disabled: run realtime-only path for all cycles.
                    heavy_scan = False
                    ...
                    if not COMMON_WARMUP_WS and not COMMON_WARMUP_DONE:
                        ...
                        if not COMMON_WARMUP_DONE:
                            ...
                            continue
                        now_ts = time.time()
                        today_kst = _date_str_kst(now_ts)
                        recently_notified = False
                        if _COMMON_WARMUP_NOTIFY_TS_MEM and (now_ts - _COMMON_WARMUP_NOTIFY_TS_MEM) < COMMON_WARMUP_NOTIFY_COOLDOWN_SEC:
                            recently_notified = True
                        meta = state.setdefault("_meta", {}) if isinstance(state, dict) else {}
                        last_ts = meta.get("common_warmup_last_notify_ts") if isinstance(meta, dict) else None
                        if isinstance(last_ts, (int, float)) and (now_ts - float(last_ts)) < COMMON_WARMUP_NOTIFY_COOLDOWN_SEC:
                            recently_notified = True
                        already_today = False
                        if isinstance(meta, dict):
                        ...
                        if (not already_today) and not recently_notified and not gap_inflight and not gap_forced:
                            try:
                                ...
                                pass
                            if isinstance(meta, dict):
                                meta["common_warmup_notified"] = True
                                meta["common_warmup_notified_day"] = today_kst
                    ...
                    if do_prefetch:
                        ...
                        if ST_FLIP_V1_ENABLED and st_flip_universe:
                            st_min = max(ST_FLIP_LOOKBACK, ST_FLIP_EMA_LEN + 5, ST_FLIP_EMA_LONG_LEN + 5, ST_FLIP_ST_PERIOD + 5, 120)
                            tf_key = str(ST_FLIP_LTF or "1m")
                            if tf_key in ltf_plan:
                        ...
                        if BULL_PULLBACK_LONG_V1_ENABLED and BullPullbackLongConfig:
                            ...
                            ltf_min = max(int(bcfg.rsi_len) + 10, int(bcfg.ema_exec_mid) + 10, 80)
                            ltf_plan.setdefault(str(bcfg.tf_exec or "1m"), ltf_min)
                        if ltf_plan:
                    ...
                    srp_result = {}
                    srp_thread = None
                    bull_pullback_result = {}
                    bull_pullback_thread = None
                    dtfx_result = {}
                    dtfx_thread = None
                    ...
                    if SRP_ST_REGIME_PULLBACK_V1_ENABLED and (not heavy_scan):
                        ...
                        srp_thread.start()
                    if SR_PRO_SHORT_V1_ENABLED and new_3m_bar:
                        sr_pro_thread = threading.Thread(
                            target=lambda: sr_pro_result.update(
                                _run_sr_pro_short_v1_cycle(
                    ...
                    print(
                        f"[universe] rule=qVol>={int(shared_min_qv):,} sort=decline(pct) topN={shared_top_n} anchors={anchors_disp} "
