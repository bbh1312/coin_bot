# ST_FLIP_V1 백테스트 지침서 (실매매 동기화 기준)
...
### 룰
- 한 봉에서 TP와 SL이 동시에 충족되면 SL을 우선 처리한다.
- ST_FLIP_V1 백테스트에서는 이 규칙이 적용 완료됨.
...
```
python engines/st_flip_v1/run_backtest.py \
  --days 3 \
  --universe common \
  --use-confirmed \
  --use-live-cache \
...
- 필터
  ...
  - Volume: `vol >= SMA(20) * 1.5`
  - RSI: `30~70` 구간만 허용
- TP/SL: `ENGINE_EXIT_OVERRIDES.ST_FLIP_V1` 기준 (현재 롱/숏 모두 2%)
...
2) `logs/common_warmup/ohlcv/*_3m.csv` 타임스탬프가 최신인지 확인
