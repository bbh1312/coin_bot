...
- ADX(14) > 20
- SuperTrend 추세가 LONG
- 볼린저 과열 필터: 15m 종가가 BB 상단(20,2) 초과면 스킵
- RSI 필터: RSI(14) >= 70 이면 스킵
- ATR 이격: |close - ST| > ATR(14) * 2.5 이면 스킵
- EMA 이격: EMA7 > EMA20 * 1.03 이면 스킵
- 거래량/윗꼬리: 전봉 대비 거래량 2배 이상 + 윗꼬리 발생 시 스킵
...
- 볼린저 과열 필터: 15m 종가가 BB 하단(20,2) 미만이면 스킵
- RSI 필터: RSI(14) <= 30 이면 스킵
- ATR 이격: |close - ST| > ATR(14) * 2.5 이면 스킵
- EMA 이격: EMA7 < EMA20 * 0.97 이면 스킵

## 청산 로직 (실매매)
- SL: 진입 직후 거래소 STOP_MARKET 주문으로 SuperTrend 라인에 스탑 설정
- BE 이동: 1.5R 도달 시 SL을 진입가(BE)로 이동
