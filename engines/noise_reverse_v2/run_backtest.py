...
NOISE_REVERSE_DISPARITY_PCT = 0.05
...
def main() -> None:
    ...
    log_dir = os.path.join(ROOT_DIR, "logs", "noise_reverse_v2", "backtest")
    _ensure_dir(log_dir)
    ...
    _bt_log(
        "[run] mode=noise_reverse_v2 days=%d start_ms=%d end_ms=%d universe=%s cooldown=%d tp=%.3f sl=%.3f"
        % (
            args.days,
            start_ms,
            end_ms,
        ...
        for i in range(min_len, end_idx):
            ...
            if open_pos:
                ...
                if exit_reason:
                    ...
                    if args.verbose:
                        _bt_log(
                            "NOISE_V2_EXIT sym=%s side=%s reason=%s pnl=%.4f"
            ...
            # use confirmed candle with extra delay (i-2) when enabled
            sig_row = df.iloc[i - sig_offset] if args.use_confirmed else row
            ...
            if vol_spike and isinstance(hi100, (int, float)) and isinstance(lo100, (int, float)) and isinstance(ma20, (int, float)):
                if c_sig > hi100 and c_sig > ma20 * (1.0 + float(disparity_pct)):
                    entry_side = "LONG"
                elif c_sig < lo100 and c_sig < ma20 * (1.0 - float(disparity_pct)):
                    entry_side = "SHORT"
            ...
            if args.verbose:
                _bt_log("NOISE_V2_ENTRY sym=%s side=%s entry=%.6g" % (symbol, entry_side, entry_px))
