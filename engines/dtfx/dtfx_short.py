...
class DTFXShortEngine:
    """
    DTFX 숏 전략의 상태를 관리하고, 캔들 데이터를 기반으로 상태 전환을 처리합니다.
    (유동성 스윕 -> 구조 전환 -> 되돌림 -> 진입)
    """
    ...
    def _log_and_create_signal(self, state: Dict[str, Any], candle: Candle, event: str, new_fsm_state: DTFXState, context: Dict[str, Any]) -> Signal: # Changed type hint
        ...
        signal = Signal(
            ts=candle.ts, engine="dtfx_short", symbol=state['symbol'],
    ...
    def _change_state(self, state: Dict[str, Any], new_state: DTFXState): # Changed new_state type hint
        ...
        if new_state == DTFXState.IDLE: # Use Enum
            for key in ['sweep_context', 'mss_context', 'entry_zone', 'last_swing_high', 'mss_break_level']:
                state[key] = None
    ...
    def _get_zone_skipped_reason_short(self, ltf: List[Candle], mss_context: Dict[str, Any], zone: Optional[Dict[str, Any]], zone_type: Optional[str]) -> Optional[str]:
        if not zone:
            if zone_type == "ob":
                return "no_ob_found"
            elif zone_type == "fvg":
    ...
    def _overlaps_short(self, candle: Candle, zone_lo: float, zone_hi: float) -> bool:
        return candle.h >= zone_lo and candle.l <= zone_hi
    ...
    def _handle_idle(self, state: Dict[str, Any], candle: Candle, ltf: List[Candle], mtf: List[Candle], htf: List[Candle]) -> Optional[Signal]:
        """IDLE 상태: 매수측 유동성 스윕을 감시합니다."""
        recent_highs = [s for s in state['swings_ltf'] if s.type == 'high']
        if not recent_highs:
            return None
        state['last_swing_high'] = recent_highs[-1]
        ...
        sweep_context = liquidity.check_buy_side_sweep(
            candle, state['last_swing_high'], atr,
    ...
    def _handle_sweep_seen(self, state: Dict[str, Any], candle: Candle, ltf: List[Candle], mtf: List[Candle], htf: List[Candle]) -> Optional[Signal]:
        """SWEEP_SEEN 상태: 하방 시장 구조 변화(MSS)를 감시합니다."""
        ...
        new_lows = [s for s in state['swings_ltf'] if s.type == 'low' and s.ts > sweep_ts]
        if not new_lows: return None
        state['mss_break_level'] = new_lows[-1]
        ...
        soft_passed = candle.l <= soft_line
        failed = []
        if close_passed:
            failed = []
        elif soft_passed:
        ...
        if close_passed:
            mss_context = structure.check_mss_down(
    ...
    def _handle_mss_confirmed(self, state: Dict[str, Any], candle: Candle, ltf: List[Candle], mtf: List[Candle], htf: List[Candle]) -> Optional[Signal]:
        ...
        if mss_candle_idx == -1:
            self._log_and_create_signal(state, candle, "INVALIDATED", DTFXState.IDLE, {"reason": "mss_candle_not_found", "from_state": DTFXState.MSS_CONFIRMED.value})
            self._change_state(state, DTFXState.IDLE)
            return None

        found_zone = None
        found_zone_type = None # Track type of zone found
        ob_zone = zones.find_order_block(ltf, mss_candle_idx, 'down', self.params.get('zone_ob_atr_max', 1.0)) # For short, dir is 'down'
        if ob_zone:
            found_zone = ob_zone
            found_zone_type = "ob"
    ...
    def _handle_retrace_zone_set(self, state: Dict[str, Any], candle: Candle, ltf: List[Candle], mtf: List[Candle], htf: List[Candle]) -> Optional[Signal]:
        ...
        if state['timeout_counter'] > self.params['timeout_mss_to_touch']:
            ...
            self._log_and_create_signal(state, candle, "TIMEOUT", DTFXState.IDLE, {"reason": "timeout_mss_to_touch", "from_state": DTFXState.ZONE_SET.value})
            self._change_state(state, DTFXState.IDLE)
            return None
        ...
        # Check for invalidation (close above sweep_high + deadband for 2 consecutive candles)
        if candle.c > invalidate_price_level:
            state['invalidate_consecutive_counter'] += 1
            if state['invalidate_consecutive_counter'] >= 2:
                self._log_and_create_signal(state, candle, "ZONE_INVALIDATED", DTFXState.IDLE, {"reason": "consecutive_close_above_deadband", "from_state": DTFXState.ZONE_SET.value, "deadband_price": invalidate_price_level})
                self._log_and_create_signal(state, candle, "INVALIDATED", DTFXState.IDLE, {"reason": "consecutive_close_above_deadband", "from_state": DTFXState.ZONE_SET.value, "deadband_price": invalidate_price_level})
                self._change_state(state, DTFXState.IDLE)
                return None
        else:
        ...
        zone_lo = zone['lo'] - pad
        zone_hi = zone['hi'] + pad
        overlap = self._overlaps_short(candle, zone_lo, zone_hi)
        ...
        if overlap:
            ...
            touch_price = candle.h # Use candle high as the interaction price

            if zone_lo <= touch_price <= zone_hi:
                touch_type = "TOUCH"
            elif candle.l <= zone_hi and touch_price > zone_hi:
                touch_type = "PIERCE"
            else:
                return None

            # Check MSS direction alignment
            mss_dir_aligned = state['mss_context']['dir'] == 'down' # For short engine
            ...
            if candle_range <= eps:
                ...
                wick_ratio_value = upper_wick / candle_range
                doji_block = False
            ...
            if candle_range <= eps:
                ...
                close_ratio_value = (candle.h - candle.c) / candle_range
            ...
            self._change_state(state, DTFXState.COOLDOWN)
            return signal
        return None
