...
from engines.div15m_short.engine import Div15mShortEngine, Div15mShortEvent
...
def _append_csv(path: str, event: Div15mShortEvent) -> None:
...
def parse_args():
    parser = argparse.ArgumentParser(description="DIV15m Short Divergence Backtest")
    parser.add_argument("--symbols", type=str, default="BTC/USDT:USDT")
    parser.add_argument("--start", type=str, required=True)
    parser.add_argument("--end", type=str, required=True)
    parser.add_argument("--tf", type=str, default="15m")
    parser.add_argument("--out-csv", type=str, default="logs/div15m_short/div15m_short_signals.csv")
    parser.add_argument("--out-trades", type=str, default="logs/div15m_short/div15m_short_trades.csv")
    parser.add_argument("--log-path", type=str, default="logs/div15m_short/backtest.log")
...
def main() -> None:
    ...
    cfg = Div15mShortConfig()
    engine = Div15mShortEngine(cfg)
    ...
    for symbol in symbols:
        before = _snapshot()
        log_fn(f"[div15m-short] fetch sym={symbol} tf={args.tf} start={start_ms} end={end_ms}")
        rows = _fetch_ohlcv_all(exchange, symbol, args.tf, start_ms, end_ms)
        df = _to_df(rows)
        if df.empty or len(df) < 50:
            log_fn(f"[div15m-short] empty_or_short sym={symbol} rows={len(df)}")
            continue
        ...
        macd_hist = engine._macd_hist(close, cfg.MACD_FAST, cfg.MACD_SLOW, cfg.MACD_SIGNAL)
        ...
        def _handle_entry_ready(event: Div15mShortEvent, ts: int, idx: int, c: float, h: float, l: float) -> None:
            nonlocal pending, trade, cooldown_until
            if event is None or event.event != "ENTRY_READY":
                return
            if trade is not None:
                trade_stats["skipped_in_pos"] += 1
                log_fn(f"DIV15S_TRADE_SKIP sym={symbol} reason=in_position")
                if args.out_csv:
                    _append_csv(
                        args.out_csv,
                        Div15mShortEvent(
            ...
            if idx <= cooldown_until:
                ...
                log_fn(f"DIV15S_TRADE_SKIP sym={symbol} reason=cooldown")
                if args.out_csv:
                    _append_csv(
                        args.out_csv,
                        Div15mShortEvent(
            ...
            if idx + 1 >= len(df) and cfg.ENTRY_MODE == "NEXT_OPEN":
                ...
                log_fn(f"DIV15S_TRADE_SKIP sym={symbol} reason=no_next_bar")
                if args.out_csv:
                    _append_csv(
                        args.out_csv,
                        Div15mShortEvent(
        ...
        for idx in range(len(df)):
            ...
            if trade is not None:
                ...
                if exit_reason:
                    ...
                    pnl_pct = _calc_pnl_short(trade.entry_px, exit_px, cfg.FEE_RATE, cfg.SLIPPAGE_PCT)
            ...
            event = engine.process_candidate(symbol, df, idx, rsi, ema_fast, ema_slow, ema_regime, log_fn, macd_hist)
        ...
        if trade is not None:
            ...
            pnl_pct = _calc_pnl_short(trade.entry_px, last_close, cfg.FEE_RATE, cfg.SLIPPAGE_PCT)
            pnl_usdt = pnl_pct * cfg.POSITION_SIZE_USDT
        ...
        log_fn(
            "[div15m-short-confirm-fail] "
            f"no_ob={c['confirm_fail_no_ob']} "
            f"no_reject={c['confirm_fail_no_reject']} "
            f"spike_block={c['confirm_fail_spike']} macd_block={c['macd_block']}"
        )
        log_fn(
            "[div15m-short-trades] "
    ...
    log_fn(
        "[div15m-short-confirm-fail] "
        f"no_ob={total['confirm_fail_no_ob']} "
        f"no_reject={total['confirm_fail_no_reject']} "
        f"spike_block={total['confirm_fail_spike']} macd_block={total['macd_block']}"
    )
